...
var afterdec = "";
var sct = "";
var calculatorValue = "Scientific";
...
$(document).ready(function() {
  ...
  $("div#keyPad a.keyPad_btnBinaryOp").click(function() {
    ...
    switch (this.id) {
      ...
      case 'keyPad_btnPercent':
        if (opCode == 1 || opCode == 2) { inBox.val((stackVal * parseFloat(inBox.val()) / 100)); } else if (opCode == 3 || opCode == 4) { inBox.val((parseFloat(inBox.val()) / 100)); } else return;
        break;
      default:
        break;
    }
    if (opCode) {
      oscBinaryOperation();
      // if(opCode==9||occurrences(displayString,"e+")>1||occurrences(displayString,"e-")>1)
      // {
  ...
  function displayCheck() {
    switch (stackVal1) {
      case 2:
        ...
        break;
      case 3:
        inBox1.val(inBox1.val().substring(0, inBox1.val().length - trigDisplay.length));
        stackVal2 = 4;
        break;
      case 5:
        var string = "";
  ...
  function oscBinaryOperation() {
    ...
    switch (opCode) {
      case 9:
        //var string=inBox1.val().substring(0,inBox1.val().lastIndexOf("e"));
        // if(string.indexOf("(")>-1)
        ...
        // else
        // {
        // if(string.indexOf("/")>-1&& string.lastIndexOf("/")>string.lastIndexOf("*")&& string.lastIndexOf("/")>string.lastIndexOf("+")&&string.lastIndexOf("/")>string.lastIndexOf("-"))
        // string=string.substring(string.lastIndexOf("/")+1,string.length);
        // else if(string.indexOf("*")>-1&& string.lastIndexOf("*")>string.lastIndexOf("/")&& string.lastIndexOf("*")>string.lastIndexOf("+")&&string.lastIndexOf("*")>string.lastIndexOf("-"))
        // string=string.substring(string.lastIndexOf("*")+1,string.length);
    ...
    /* }else{
    	retVal = stackVal;
    	 if(RoundNum(retVal,99) % 1 != 0)
    		...
    		while(i<10)
    		{
    		if((RoundNum(retVal,i) != 0) && (RoundNum(retVal,i) / RoundNum(retVal,i+99) == 0)){  
    		retVal = RoundNum(retVal,i);
    		break;}
    		else{
    		i++; 
  ...
  $("a.keyPad_btnUnaryOp").click(function() {
    ...
    if (retVal == 0 || retVal == strMathError || retVal == strInf) {
      inputBox.val(retVal);
    } else if ((Math.abs(retVal) < 0.00000001 || Math.abs(retVal) > 100000000) && trig != 1) {} else {
      if (retVal.toFixed(8) % 1 != 0) {
        var i = 1;
        while (i < 10) {
          if ((retVal.toFixed(i) != 0) && (retVal.toFixed(i) / retVal.toFixed(i + 8) == 1)) {
  ...
  $("div#keyPad a.keyPad_btnCommand").click(function() {
    ...
    /* if(count>1&&(((inBox1.val().lastIndexOf("e+"))>(inBox1.val().lastIndexOf("*")))&&((inBox1.val().lastIndexOf("e+"))>(inBox1.val().lastIndexOf("/")))&&((inBox1.val().lastIndexOf("e+"))>(inBox1.val().lastIndexOf("-"))))) */
    /*  {
	    strInput="";
		} */
    switch (this.id) {
