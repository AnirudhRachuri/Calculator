...
function readSysInstructionsXMLQuizPage(QPxml,xml,orgId,mockId,isResumed){
	...
	$("#footer").html("Version : " +$(xml).find("Version").text()+"</div>");
...
function renderQuestions(xml,selectorElement){
	...
	$(xml).find(selectorElement).each(function(){
		...
		$(this).find("section").each(function(){
				...
				$(this).find("question").each(function(){
					...
					if(questionShuffling && (comprehensionId!=0 || laqId!=0)){
						if(comprehensionId!=0){
							...
							compreQuestionArr[compQuesCounter].quesBean.push(new questions(quesId, quesType, comprehensionId, laqId, laqParentId, correctAnswer, allottedMarks, negMarks, quesKeyPair, keyboardType, typingType, answerType, isCaseSensitive, isEvaluationRequired, quesParam, quesLangBeans, options, hint, testCasePair , paragraphDisplay, programmingSkeletalCode, alphaWordLimit,mockVar.storeCandResponse));
						}else if(laqId!=0){
						...
						questionArr.push(new questions(quesId, quesType, comprehensionId, laqId, laqParentId, correctAnswer, allottedMarks, negMarks, quesKeyPair, keyboardType, typingType, answerType, isCaseSensitive, isEvaluationRequired, quesParam, quesLangBeans, options, hint, testCasePair,paragraphDisplay, programmingSkeletalCode, alphaWordLimit,mockVar.storeCandResponse));
					}
				});
			...
			iOAP.secDetails[secCounter] = new secBean(secName,answered,notanswered,marked,markedAndAnswered,isOptional,secType,questionArrSection, hasOptionalQuestion,sectionId,difficultyLevel,difficultyLabels);
...
//Need to implement
	...
	var compreId = '',laqId = '',langId = '',quesText = '';
	$(xml).find('comprehensions').find('comprehension').each(function(){
		...
		mockVar.compreLaqQues.push(new compreLaqQuesBean(compreId,langBeanArr));
	});
	$(xml).find('laqs').find('laq').each(function(){
		...
		mockVar.compreLaqQues.push(new compreLaqQuesBean(laqId,langBeanArr));
	});
...
function fillQuesContent(){
	...
	if(mockVar.curQuesBean.quesType == "SA" || mockVar.curQuesBean.quesType == "LA@@SA" || mockVar.curQuesBean.quesType == "COMPREHENSION@@SA"){
		...
		// alphanumeric textarea
			...
			$('#answerArea').after('<div style="padding-left: 4%;"> <span id="noOfWords" name="noOfWords" style="font-size: 12px;color:#2F72B7;font-weight: bold;">'+mockVar.curQuesBean.typedWord+'</span><span id="maxalert" name="maxalert"  style="font-size: 10px;color:#FF1919;font-weight: bold;"></span><div> <textarea rows="7" cols="75" name="option"  autocomplete="off" id="answer" class="answer" onpaste="return false"; onkeydown="disableTab(event); allowSAInputsForMultiLang(event); " onkeyup="word_count(); alphaWordLimit(event);">'+mockVar.curQuesBean.quesParam.answer+'</textarea></div></div>');
			wordCountCheck();
			$('#clearResponse').hide();
...
function fillMCQQues (quesTxt,quesOpts,answer){
	var str = "<div id='quesAnsContent' class='Ans_Area ZeroLevelZoom' style='overflow:auto;'>";
	...
	for(var i=0;i<quesOpts.length;i++){
		if(!mockVar.showOptionsHorizontally)
			str+= '<label>';
		str += "<span><input type='radio' onMouseDown='this.check = this.checked'  onClick='if (this.check) this.checked = false' class='answer' name='answers' value='"+quesOpts[i].optId+"' ";
...
function fillMSQQues (quesTxt,quesOpts,answer){
	...
	for(var i=0;i<quesOpts.length;i++){
		if(!mockVar.showOptionsHorizontally)
			str += '<label>';
		str += "<span><input type='checkbox' class='answer' name='answers' value='"+quesOpts[i].optId+"' ";
		for(var j=0;j<answers.length;j++){
			if(answers[j]!="" && quesOpts[i].optId==answers[j])
				str += "checked";
		}
...
function fillProgramingQues(ques){
	...
	str +="<div class='divHeader'><b><span class='questionNumber'>"+mockLabels.questionNo+"</span>. "+eval(iOAP.curQues+1)+"</b></div>";
	str += '<div id="quesAnsContent" class="Ans_Area ZeroLevelZoom">'+ques.quesLangBeans[mockVar.langIndex].quesText + ' </div>';
	...
	str +='<div style="float:right;margin-top:-23px;margin-right:12px;"><img alt="" src="images/ChangeEditor.png" onclick="changeEditor();" style="cursor:pointer;width:20px;" title="Change the Editor"></div>';
	...
	str +='<div style="float:right;margin-top:-23px;margin-right:12px;"><img alt="" src="images/ChangeTextArea.png" onclick="changeTextArea();" style="cursor:pointer;width:20px;" title="Change the Textarea"></div>';
...
function fillLAQues(laQues, parentQuesIndex){
	...
	str += (mockVar.laQName.length>0)?('<table width="100%"><tr><td><div  style="border-bottom:1px solid #dbdbdb;height:26px"><b>'+mockVar.laQName+'</b></div></td></tr></table><div id="warning"></div>'):'';
...
function fillSubjectiveQues(quesTxt){
	var str = "<div id='quesAnsContent' class='Ans_Area ZeroLevelZoom' style='width:98%; margin-left:5px; font-family:Arial,verdana,helvetica,sans-serif; overflow:auto'> "+quesTxt + "</div>";
	return str;
}
...
function fillLAQuesNumber(ques){
	var str = '<div id="Questn_Innr_Div_section"><div id="Subjt_Div"><b><span class="questionNumber">'+mockLabels.questionNo+'</span>. '+eval(iOAP.curQues+1);
...
function fillSections(){
	...
	for(var i=0;i<iOAP.secDetails.length ;i++){
		...
		//	str += '<span class="subject_instruction_icon" id='+i+' onclick="a(this.id);"></span></div>';
		str +='</div>';
	
	/*	if(incr=='0')
		{
			str += '<span class="subject_information_div" id="id'+i+'" style="display:none" ><table>';
		}
		else if(idGbl=='id'+i && incr!='0')
		{
			str += '<span class="subject_information_div" id="id'+i+'" style="" ><table>';
		}
		else
		{
			str += '<span class="subject_information_div" id="id'+i+'" style="display:none"><table>';
		...
		str += '<div class="subject_name">'+iOAP.secDetails[i].secName+'</div>';
		str += '</table>';
		str += '<table class="notation_type_description diff_type_notation_area_inner" >';
		str += '<tr class="notation_typeDiv"><td><span class="answered">'+answeredQuestions+'</span></td><td  class=\"type_title\">'+mockLabels.answered+'</td></tr>';
		str += '<tr class="notation_typeDiv"><td><span  class="not_answered">'+notAnsweredQuestions+'</span></td><td  class=\"type_title\">'+mockLabels.notAnswered+'</td></tr>';
		str += '<tr class="notation_typeDiv"><td><span class="review">'+markedQuestions+'</span></td><td  class=\"type_title\">'+mockLabels.markReview+' </td></tr>';
		str += '<tr class="notation_typeDiv"><td ><span class="not_visited">'+notAttemptedQuestions+'</span></td><td  class=\"type_title\">'+mockLabels.notAttempted+'</td></tr>';
		str += '<tr class="notation_typeDiv"><td ><span class="review_marked">'+markedAndAnsweredQuestions+'</span></td><td  class=\"type_title\">'+mockLabels.markAnsTitle+'</td></tr>';
		str += '</table></span>';
		...
		$('.subject_instruction_icon').hide();
	}
	else
		$('.subject_instruction_icon').show();*/
...
function sectionWidthCalculate()
{
			...
			jQuery(".subject-selection").css("width",calculatesubjectwidth);
...
function optSecUncheck(secId,event){
	...
	str +='<tr><td style="text-align:right;padding-right:2px"><input onclick="resetSection('+secId+');afterResetSection();" type="button" class="btn btn-primary" style="margin-right:20px" value="'+mockLabels.reset+'"/></td><td  style="text-align:left;padding-left:2px"><input onclick="showModule(';
	str +="'questionCont'";
	str +=')" type="button" class="btn btn-primary" style="margin-left:20px" value="'+mockLabels.back+'"/></td></tr></table></center>';
...
function secChangeConfirmation(){
	...
	str +='<div><span style="text-align:right;padding-right:2px"><input onclick="confirmChangeSec()" style="margin-right:20px" type="button" class="btn btn-primary" value="'+mockLabels.reset+'"/></span><span  style="text-align:left;padding-left:2px;"><input onclick="showModule(';
	str +="'questionCont'";
	str +=')" type="button" class="btn btn-primary" style="margin-left:20px" value="'+mockLabels.back+'"/></span></div></center>';
	$("#sectionSummaryDiv").html(str);
	showModule('sectionSummaryDiv');
}
...
function confirmChangeSec(){
	...
	if(sections.length>1){
		...
		str += "'"+secIds+"'";
		str += ')" type="button" style="margin-right:20px" class="btn btn-primary" value="'+mockLabels.reset+'"/></td><td  style="text-align:left;padding-left:2px"><input onclick="showModule(';
		str +="'questionCont'";
		str +=')" type="button" style="margin-left:20px" class="btn btn-primary" value="'+mockLabels.back+'"/></td></tr></table></center>';
		$("#sectionSummaryDiv").html(str);
...
function moveToFeedback(){
	...
	if(mockVar.isFeedBackRequired == "NO"){
		...
		window.location.href = "FeedBack.html?"+mockVar.orgId +"@@"+mockVar.mockId+"@@"+mockVar.subscribedFor+"#";
	}
}
...
function finalSubmit(type){
	...
	if(mockVar.storeCandResponse==1){
	if(mockVar.groups.length<=(mockVar.currentGrp+1)){
...
function fnSubmit(action){
	...
	if(action != "SKIP"){
		...
		if(selectedAnswer.length>selectedAnswer1.length){
...
function loadCalculator(){
	...
	if(mockVar.showCalculator=="SCIENTIFIC"){
	 ...
	 jQuery("#loadCalc").css("top",51).css("left","35%");
	$('#loadCalc').show();
...
function showScoreCard(){
	...
	for(var groupNo=0;groupNo<mockVar.groups.length;groupNo++){
		...
		if(temp_iOAP.isTypingGroup){
			if(mockVar.groups.length>1)
				typingStr += "<br><span style=''><b>"+mockVar.groups[groupNo].groupName+"</b></span>";
			typingStr += "<table class='score_card_table' cellspacing=0 width='80%' align='center'>";
			...
			if(mockVar.groups.length>1)
				str += "<br><span style=''><b>"+mockVar.groups[groupNo].groupName+"</b></span>";
			str += "<table class='score_card_table' cellspacing=0 width='80%' align='center'>";
...
function loadLabel(){
	...
	if(mockVar.storeCandResponse==1){
		$('.candOriginalName').html(mockVar.candName);
			...
			mockLabels.candidate = "John Smith";
...
function loadIndexLabels(){
	...
	$('#LoginPageHeader').html($(xml).find('LoginPage4').text());
	$('#passwordLabel').html($(xml).find('GuestMode2').text());
	$('#changeLang').html($(xml).find('ChangeLanguage').text());
	$('#signInLabel').html($(xml).find('TestExpire3').text());
...
function getCandIdFromCookie(){
	//setCandCookie();
	var i,x,y,defLang="",langName="",candId="11111",candMasterId="",qpId="",candName = " ",candidate_Id="",subscribedFor = '',ARRcookies=document.cookie;
...
//////console.log(mockVar.storeCandResponse);
	if(false && dummyAuditJson.length > 0 && mockVar.storeCandResponse == 1 ){
...
var url = document.URL;
 ...
 if(url.indexOf("quiz.html") > 0 && !isfinalSubmit && !isPlayer && mockVar.storeCandResponse == 1){
	...
	if(!focused){
		...
		$("#seterror").html("Your Assessment is interrupted due to one of the following reasons: <br></br> <ol><li> You are trying to minimize OR toggle Assessment console </li><li> You have pressed special keys from your keyboard which are not allowed during Assessment. </li> <li> You have tried to move out of Assessment console which is not allowed.</li> <li> You have tried to refresh the page.</li></ol> All the interruptions during the Assessment are recorded.If you exceed the number of allowed interruptions, your Assessment will be locked and can only be unlocked by Administrator.<br></br><span style='color:red;text-transform:uppercase;font-weight:bold'>Your Assessment Window will close down in <br> <span id='interruptionTimer' style='color:black;font-size:30px;margin-left:140px'>5</span> seconds <br> Click OK to resume</span>");
